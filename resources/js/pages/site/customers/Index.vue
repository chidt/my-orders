...
const breadcrumbs = [
  ...
  { title: 'Quản lý khách hàng', href: `/${props.site.slug}/customers`, current: true },
];
...
const openDeleteDialog = (customer: CustomerListProps['customers']['data'][number]) => {
...
const confirmDelete = () => {
  ...
  router.delete(`/${props.site.slug}/customers/${customerToDelete.value.id}`, {
    preserveScroll: true,
    onFinish: () => {
    ...
    <div class="px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div class="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          ...
          <p class="mt-1 text-2xl font-semibold text-gray-900">{{ statistics.individual }}</p>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          <p class="text-sm text-gray-500">Khách hàng doanh nghiệp</p>
          <p class="mt-1 text-2xl font-semibold text-gray-900">{{ statistics.business }}</p>
        </div>
      </div>
      ...
      <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div v-if="customers.data.length === 0" class="py-12 text-center">
          <ContactRound class="mx-auto mb-4 h-12 w-12 text-gray-300" />
          <h3 class="text-lg font-medium text-gray-900">Chưa có khách hàng nào</h3>
          <p class="mt-2 text-sm text-gray-500">Bắt đầu bằng cách thêm khách hàng đầu tiên.</p>
        </div>
        ...
        <div v-else>
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="customer in customers.data" :key="customer.id" class="hover:bg-gray-50">
                  <td class="px-6 py-4 text-sm">
                    ...
                    <div class="mt-1 text-xs text-gray-500">Tạo lúc: {{ formatDate(customer.created_at) }}</div>
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-700">
                    <div>{{ customer.phone }}</div>
                    <div class="text-xs text-gray-500">{{ customer.email || '-' }}</div>
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-700">{{ typeLabel(customer.type) }}</td>
          ...
          <div class="space-y-3 bg-slate-50/70 p-4 md:hidden">
            ...
            >
              <div class="mb-3 flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <p class="truncate text-sm font-semibold leading-tight text-gray-900">{{ customer.name }}</p>
                  </div>
              ...
              <p class="mb-3 text-sm text-gray-500">{{ customer.phone }}</p>

              <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span class="text-xs font-medium uppercase tracking-wide text-gray-500">Loại khách</span>
                  <div class="mt-1 text-sm font-medium leading-tight text-gray-900">{{ typeLabel(customer.type) }}</div>
                </div>
                <div>
      ...
      <DialogContent class="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <span class="font-semibold">{{ customerToDelete?.name }}</span>?
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="outline" type="button" class="cursor-pointer" @click="showDeleteDialog = false">Hủy</Button>
          <Button variant="destructive" type="button" :disabled="!!isDeleting" @click="confirmDelete">
            <span v-if="isDeleting">Đang xóa...</span>
            <span v-else class="cursor-pointer">Xóa khách hàng</span>
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
