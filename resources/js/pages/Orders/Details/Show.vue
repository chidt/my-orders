...
watch(
    () =>
        [
            props.orderDetail.status.value,
            props.orderDetail.payment_status.value,
        ] as const,
    ([statusVal, payVal]) => {
...
<template>
    ...
    >
        <div class="space-y-4 px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div
                class="grid grid-cols-1 gap-4 rounded-lg border bg-white p-4 md:grid-cols-2"
            >
                <div>
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-700">
                        ...
                        {{ orderDetail.product_item.sku }}
                    </p>
                    <p class="text-sm text-gray-600">
                        Loại: {{ orderDetail.product.type || '-' }}
                    </p>
                </div>
            </div>

            <div
                class="grid grid-cols-1 gap-4 rounded-lg border bg-white p-4 md:grid-cols-5"
            >
                <div>
            ...
            >
                    ...
                    >
                        <option
                            v-for="statusOption in statusOptions.filter(
                                (status) =>
                                    orderDetail.status.allowed_status_values.includes(
                                        Number(status.value),
                                    ),
                            )"
                ...
                <div class="space-y-2">
                    ...
                    >
                        <option
                            v-for="paymentOption in paymentStatusOptions"
            ...
            <div class="rounded-lg border bg-white p-4">
                ...
                <div class="mt-3 space-y-2">
                    ...
                    >
                        ...
                        <p v-if="item.note" class="text-gray-700">
                            Ghi chú: {{ item.note }}
                        </p>
                    </div>
                </div>
            ...
            <div class="rounded-lg border bg-white p-4">
                ...
                <p class="mt-1 text-sm">
                    Order note: {{ orderDetail.notes.order_note || '-' }}
                </p>
                <p class="mt-1 text-sm">
                    Shipping note: {{ orderDetail.notes.shipping_note || '-' }}
                </p>
            </div>
        </div>
    </AppLayout>
